...
2. **If no profile exists**: Read the vendor profile template at `${VIBE_EXTENSION_ROOT}/templates/vendor-profile-template.md` and create a new file at `projects/{project-dir}/vendors/{vendor-slug}-profile.md`. Populate all sections from the research findings. Set `Confidence` based on the depth of data gathered (high = 5+ data points, medium = 3-4, low = fewer).
...
5. **If no tech note exists**: Read the tech note template at `${VIBE_EXTENSION_ROOT}/templates/tech-note-template.md` and create a new file at `projects/{project-dir}/tech-notes/{topic-slug}.md`. Populate from research findings.
...
- **Templates** — `${VIBE_EXTENSION_ROOT}/templates/research-findings-template.md` (override at `.arckit/templates-custom/research-findings-template.md`) · `${VIBE_EXTENSION_ROOT}/templates/vendor-profile-template.md`
- **Helpers** — `${VIBE_EXTENSION_ROOT}/scripts/bash/create-project.sh` (project resolution) · `${VIBE_EXTENSION_ROOT}/scripts/bash/generate-document-id.sh` (document ID allocation)
