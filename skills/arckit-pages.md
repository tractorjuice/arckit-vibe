...
- **If no hook message** (hook unavailable or failed): fall back to manual sync and title extraction:
  1. Use **Glob** to list all `.md` files in `${VIBE_EXTENSION_ROOT}/docs/guides/` (and any subdirectories like `uk-government/`, `uk-mod/`, `roles/`)
...
- **If not found**: Read `${VIBE_EXTENSION_ROOT}/templates/pages-template.html` (default)
...
3. Replace the placeholder values **in memory** (string replacement) with actual repository details:
   ...
   - `'{{VERSION}}'` → the ArcKit version from the plugin's VERSION file (`${VIBE_EXTENSION_ROOT}/VERSION`)
...
**Remember**: You MUST read and use `${VIBE_EXTENSION_ROOT}/templates/pages-template.html` as the base for `docs/index.html`. The template is the source of truth for all HTML, CSS, and JavaScript. Only replace the `{{...}}` config placeholders with actual values.
