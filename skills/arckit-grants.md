...
- `ARC-*-REQ-*.md` in `projects/{project}/` — Requirements specification
  ...
  - If missing: warn user to run `/arckit:requirements` first, but proceed using `${args}` as the project description
...
1. **Read the template** (with user override support):
   ...
   - **If not found**: Read `${VIBE_EXTENSION_ROOT}/templates/grants-template.md` (default)

2. Before writing, read `${VIBE_EXTENSION_ROOT}/references/quality-checklist.md` and verify all **Common Checks** pass. Fix any failures before proceeding.
...
2. **If no tech note exists**: Read the tech note template at `${VIBE_EXTENSION_ROOT}/templates/tech-note-template.md` and create a new file at `projects/{project-dir}/tech-notes/{grant-slug}.md`. Populate from research findings.
...
- **Templates** — `${VIBE_EXTENSION_ROOT}/templates/grants-template.md` (override at `.arckit/templates-custom/grants-template.md`)
